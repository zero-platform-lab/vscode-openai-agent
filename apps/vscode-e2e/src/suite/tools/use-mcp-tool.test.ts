...
suite.skip("OpenAI Compatible Agent use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		...
		// Create .agent directory and MCP configuration file
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		api.on(AgentEventName.Message, messageHandler)
		...
		api.on(AgentEventName.TaskStarted, taskStartedHandler)
		...
		api.on(AgentEventName.TaskCompleted, taskCompletedHandler)
		await sleep(2000) // Wait for OpenAI Compatible Agent to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this OpenAI Compatible Agent won't load the MCP server
		...
		try {
			...
			// Clean up
			api.off(AgentEventName.Message, messageHandler)
			api.off(AgentEventName.TaskStarted, taskStartedHandler)
			api.off(AgentEventName.TaskCompleted, taskCompletedHandler)
		}
	})
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasAgentDir = responseText.includes(".agent")

			// At least one of our test files or the .agent directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasAgentDir,
				`MCP server response should contain our test files or .agent directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.agent'. Got: ${responseText.substring(0, 200)}...`,
			)
