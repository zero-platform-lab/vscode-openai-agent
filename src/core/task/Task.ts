...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	private async addToApiConversationHistory(message: Anthropic.MessageParam, reasoning?: string) {
		...
		if (message.role === "assistant") {
			...
			// Persist the thought signature as its own content block so converters can
			// attach it back to the correct provider-specific fields.
			if (thoughtSignature) {
	...
	async ask(
		...
		if (this.abort) {
			throw new Error(`[Agent#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[Agent#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			if (this.abort) {
				throw new Error(`[Agent#recursivelyMakeAgentRequests] task ${this.taskId}.${this.instanceId} aborted`)
			}
			...
			// This build only ships the OpenAI Compatible provider, which uses the OpenAI protocol.
			...
			try {
				...
				if (this.abort || this.abandoned) {
					throw new Error(
						`[Agent#recursivelyMakeAgentRequests] task ${this.taskId}.${this.instanceId} aborted`,
					)
				}
	...
	public async *attemptApiRequest(
		...
		const cleanConversationHistory = buildCleanConversationHistory(
		...
		// The former Gemini-only allowedFunctionNames restriction no longer applies (Gemini provider removed).
		const supportsAllowedFunctionNames = false
