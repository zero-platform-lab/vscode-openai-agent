...
import { type ModelInfo, openAiModelInfoSaneDefaults } from "@openai-agent/types"
...
export class VsCodeLmHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		try {
			...
			throw new Error(
				`Agent <Language Model API>: Failed to initialize handler: ${error instanceof Error ? error.message : "Unknown error"}`,
			)
		}
	}
	/**
	...
	async initializeClient(): Promise<void> {
		try {
			...
			throw new Error(`Agent <Language Model API>: Failed to initialize client: ${errorMessage}`)
		}
	}
	/**
	...
	async createClient(selector: vscode.LanguageModelChatSelector): Promise<vscode.LanguageModelChat> {
		try {
			...
			const errorMessage = error instanceof Error ? error.message : "Unknown error"
			throw new Error(`Agent <Language Model API>: Failed to select model: ${errorMessage}`)
		}
	}
	...
	private async getClient(): Promise<vscode.LanguageModelChat> {
		if (!this.client) {
			...
			try {
				...
				throw new Error(`Agent <Language Model API>: Failed to create client: ${message}`)
			}
		}
	...
	override async *createMessage(
		...
		try {
			...
			const requestOptions: vscode.LanguageModelChatRequestOptions = {
				justification: `Agent would like to use '${client.name}' from '${client.vendor}', Click 'Allow' to proceed.`,
			...
			if (error instanceof Error) {
				...
				throw new Error(`Agent <Language Model API>: Response stream error: ${errorDetails}`)
			} else {
				// Fallback for unknown error types
				const errorMessage = String(error)
				console.error("Agent <Language Model API>: Unknown stream error:", errorMessage)
				throw new Error(`Agent <Language Model API>: Response stream error: ${errorMessage}`)
			}
		}
	}
	...
	override getModel(): { id: string; info: ModelInfo } {
		if (this.client) {
			...
			for (const [prop, value] of Object.entries(requiredProps)) {
				if (!value && value !== 0) {
					console.warn(`Agent <Language Model API>: Client missing ${prop} property`)
				}
			}
